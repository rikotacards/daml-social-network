...
```
daml build
daml codegen js .daml/dist/daml-social-network-0.1.0.dar -o ui/daml.js
```
...
Deploying `daml-social-network` to the hosted Daml platform
...
```
daml build -o daml-social-network.dar
```
...
DAR `daml-social-network.dar` you have just created.

To upload the UI, create a ZIP file containing all your UI assets by executing
```
daml build
daml codegen js .daml/dist/daml-social-network-0.1.0.dar -o ui/daml.js
(cd ui && npm install && npm run-script build && zip -r ../daml-social-network-ui.zip build)
```
...
(`daml-social-network-ui.zip`) you have just created and publish it.

To see your deployed instance of `daml-social-network` in action, follow the
...
- `daml.js/daml-social-network-0.1.0/src/User.ts` (once you've generated it):
